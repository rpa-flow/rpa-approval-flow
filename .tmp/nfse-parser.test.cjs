...
function collectPathValues(value, path) {
  ...
  return collectPathValues(value[current], rest);
}
function textByPath(value, path) {
  const values = collectPathValues(value, path).map(text).filter((item) => Boolean(item));
  return values.length ? values.join(" | ") : void 0;
}
function firstTextByPath(value, paths) {
...
function groupExtrasByFieldName(extras) {
  const grouped = {};
...
function simplifyExtrasByFieldName(extras) {
...
function parseNFSeXml(xml) {
  ...
  if (!infNfse) {
    throw new Error("XML inv\xE1lido: bloco infNFSe n\xE3o encontrado.");
  }
  const rawId = text(infNfse.Id) ?? "";
  const numericId = rawId.replace(/\D/g, "");
  if (numericId.length < 44 || numericId.length > 50) {
    throw new Error("Identificador inv\xE1lido: esperado c\xF3digo num\xE9rico entre 44 e 50 d\xEDgitos.");
  }
...
// src/lib/nfse-parser.test.ts
var baseId = "NFS12345678901234567890123456789012345678901234";
(0, import_node_test.default)("known fields are mapped", () => {
  ...
  import_strict.default.equal(parsed.numeroNota, "10");
  import_strict.default.equal(parsed.serie, "70000");
  import_strict.default.equal(parsed.valorServico, "100.50");
});
(0, import_node_test.default)("maps service description from DPS service code", () => {
  ...
  const parsed = parseNFSeXml(xml);
  import_strict.default.equal(parsed.descricaoServico, "Servicos de manutencao industrial");
  import_strict.default.equal(parsed.extras["NFSe.infNFSe.DPS.infDPS.serv.cServ.xDescServ"], void 0);
});
(0, import_node_test.default)("unknown fields go to extras", () => {
  ...
  const parsed = parseNFSeXml(xml);
  import_strict.default.equal(parsed.extras["NFSe.infNFSe.DPS.infDPS.campoDesconhecido"], "xyz");
  import_strict.default.equal(parsed.extras["NFSe.infNFSe.DPS.infDPS.itens[0].descricao"], "A");
  import_strict.default.equal(parsed.extras["NFSe.infNFSe.DPS.infDPS.itens[1].descricao"], "B");
});
(0, import_node_test.default)("mapped fields are not duplicated in extras", () => {
  const xml = `<NFSe><infNFSe><Id>${baseId}</Id><nNFSe>10</nNFSe><DPS><infDPS><serie>70000</serie></infDPS></DPS></infNFSe></NFSe>`;
  const parsed = parseNFSeXml(xml);
  import_strict.default.equal(parsed.extras["NFSe.infNFSe.DPS.infDPS.serie"], void 0);
});
(0, import_node_test.default)("works with namespaces", () => {
  ...
  const parsed = parseNFSeXml(xml);
  import_strict.default.equal(parsed.serie, "70000");
  import_strict.default.equal(parsed.extras["NFSe.infNFSe.DPS.infDPS.campoLivre"], "ok");
});
(0, import_node_test.default)("simplifies extras by field name", () => {
  ...
  import_strict.default.equal(simple.codigo, "ABC");
  import_strict.default.deepEqual(simple.descricao, ["Primeira", "Segunda"]);
});
(0, import_node_test.default)("ignores technical xml/signature fields in extras", () => {
  ...
  const parsed = parseNFSeXml(xml);
  import_strict.default.equal(parsed.extras["NFSe.Signature.SignatureValue"], void 0);
  import_strict.default.equal(parsed.extras["NFSe.Signature.SignedInfo.CanonicalizationMethod.Algorithm"], void 0);
  import_strict.default.equal(parsed.extras["?xml.version"], void 0);
  import_strict.default.equal(parsed.extras["NFSe.infNFSe.DPS.infDPS.campoLivre"], "ok");
});
