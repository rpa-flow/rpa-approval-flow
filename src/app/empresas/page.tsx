...
export default function EmpresasPage() {
  ...
  return (
    <AppLayout>
      <MainHeader title="Empresas" subtitle="Gerencie nomes amigáveis por CNPJ completo da nota fiscal." />
      ...
      >
        <form onSubmit={submitCompany} className="grid gap-4 md:grid-cols-4">
          ...
          <div className="md:col-span-4 flex flex-wrap justify-end gap-2">
            {editingCompanyId && <Button type="button" variant="outline" onClick={cancelEditing}>Cancelar edição</Button>}
      ...
      <SectionCard
        ...
        description="Pesquise por nome ou CNPJ e altere rapidamente os dados usados na listagem de notas."
        actions={<span className="text-sm font-semibold text-slate-500">{isLoadingCompanies ? "Carregando..." : `${pagination.total} empresa(s)`}</span>}
      >
        ...
        <DataTable
          ...
          actions={(company) => (
            <div className="flex justify-end gap-2">
              <Button type="button" variant="outline" size="sm" onClick={() => startEditing(company)}>Editar</Button>
              <Button type="button" variant={company.active ? "destructive" : "secondary"} size="sm" onClick={() => toggleCompany(company)}>
