...
export default function DashboardPage() {
  ...
  return <AppLayout onClick={() => setMenuState(null)}>
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map((invoice) => <Fragment key={invoice.id}>
              <tr className="cursor-pointer" onClick={() => setExpandedId(expandedId === invoice.id ? null : invoice.id)}>
                ...
                <td className="px-4 py-3 text-right"><button className="btn-secondary min-h-0 px-3 py-1.5 text-sm" onClick={(e) => { e.stopPropagation(); const r = (e.currentTarget as HTMLButtonElement).getBoundingClientRect(); setMenuState({ invoice, x: Math.max(8, Math.min(r.right - 208, window.innerWidth - 248)), y: Math.min(r.bottom + 6, window.innerHeight - 260) }); }}>Ações ▾</button></td>
              </tr>
    ...
    {menuState && <div className="fixed z-50 w-[calc(100vw-1rem)] max-w-60 rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl" style={{ left: menuState.x, top: menuState.y }} onClick={(e) => e.stopPropagation()}>
      ...
      <button className="w-full rounded-xl !bg-white px-3 py-2 text-left text-sm font-semibold !text-slate-700 hover:!bg-slate-50" onClick={() => { setExpandedId(menuState.invoice.id); setMenuState(null); }}>▾ Expandir resumo</button>
